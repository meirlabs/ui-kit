...
export interface StatusPillProps extends ComponentPropsWithoutRef<"span"> {
   ...
   * pill in a list is exactly what that rule protects against.
   */
  live?: boolean;
...
export const StatusPill = forwardRef<HTMLSpanElement, StatusPillProps>(function StatusPill(
  { tone = "neutral", dot = false, live = false, icon, className, children, ...rest },
  ref,
) {
  return (
    <span ref={ref} className={cn("ml-status-pill", toneClass[tone], className)} {...rest}>
      {dot ? (
        <span
          className={cn("ml-status-pill-dot", live && "ml-status-pill-dot-live")}
          aria-hidden="true"
        />
      ) : null}
      {icon ? (
